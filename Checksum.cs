...
namespace SA2_Save_Converter
{
    class Checksum
    {
        ...
        public static byte[] MainChecksumCorrected(byte[] save, int saveConsole)
        {
            byte[] checksum = new byte[4];
            List<byte> newSave = new List<byte>();

            if (saveConsole == 0) { checksum = BitConverter.GetBytes(save.Skip(0x2844).ToArray().Select(x => (int)x).Sum()); }
            else { checksum = BitConverter.GetBytes(save.Skip(0x2844).ToArray().Select(x => (int)x).Sum()).Reverse().ToArray(); }
